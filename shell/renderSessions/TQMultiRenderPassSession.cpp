...
fragment float4 fragmentShader(
  ...
  return float4(color->color.r, color->color.g, color->color.b, 1.0) * tex;
}
)";
}
...
varying vec2 uv;
void main() {
  gl_FragColor = vec4(color, 1.0) * texture2D(inputImage, uv);
})";
}
...
static std::unique_ptr<IShaderStages> getShaderStagesForBackend(IDevice& device) {
  ...
  case igl::BackendType::OpenGL:
    return igl::ShaderStagesCreator::fromModuleStringInput(
        device, getOpenGLVertexShaderSource().c_str(), "main", "",
        getOpenGLFragmentShaderSource().c_str(), "main", "", nullptr);
  case igl::BackendType::D3D12:
    return igl::ShaderStagesCreator::fromModuleStringInput(
        device, getD3D12VertexShaderSource().c_str(), "main", "",
        getD3D12FragmentShaderSource().c_str(), "main", "", nullptr);
  }
...
void TQMultiRenderPassSession::initialize() noexcept {
  ...
  vb0_ = device.createBuffer(BufferDesc{BufferDesc::BufferTypeBits::Vertex,
                                       vertexData0, sizeof(vertexData0)}, nullptr);
  vb1_ = device.createBuffer(BufferDesc{BufferDesc::BufferTypeBits::Vertex,
                                       vertexData1, sizeof(vertexData1)}, nullptr);
  ib0_ = device.createBuffer(BufferDesc{BufferDesc::BufferTypeBits::Index,
                                       indexData, sizeof(indexData)}, nullptr);

  const VertexInputStateDesc inputDesc = {
      .numAttributes = 2,
      .attributes = {
          {.bufferIndex = 0, .format = VertexAttributeFormat::Float3,
           .offset = offsetof(VertexPosUv, position), .name = "position", .location = 0},
          {.bufferIndex = 0, .format = VertexAttributeFormat::Float2,
           .offset = offsetof(VertexPosUv, uv), .name = "uv_in", .location = 1},
      },
      .numInputBindings = 1,
  ...
  samplerState_ = device.createSamplerState(SamplerStateDesc{
...
void TQMultiRenderPassSession::update(SurfaceTextures surfaceTextures) noexcept {
  ...
  if (framebuffer0_ == nullptr) {
    ...
    const igl::TextureDesc desc1 = igl::TextureDesc::new2D(
        igl::TextureFormat::RGBA_UNorm8, dimensions.width, dimensions.height,
    ...
    framebuffer0_ = getPlatform().getDevice().createFramebuffer(
        FramebufferDesc{
            ...
            .depthAttachment = {.texture = getPlatform().getDevice().createTexture(
                TextureDesc{
  ...
  if (pipelineState0_ == nullptr) {
    pipelineState0_ = getPlatform().getDevice().createRenderPipeline(RenderPipelineDesc{
        .vertexInputState = vertexInputState_,
        .shaderStages = shaderStages_,
        .targetDesc = {
            .colorAttachments = {{.textureFormat = tex1_->getProperties().format}},
            .depthAttachmentFormat = framebuffer0_->getDepthAttachment()->getProperties().format,
        },
    ...
    pipelineState1_ = getPlatform().getDevice().createRenderPipeline(RenderPipelineDesc{
        .vertexInputState = vertexInputState_,
        .shaderStages = shaderStages_,
        .targetDesc = {
            .colorAttachments = {{.textureFormat = framebuffer1_->getColorAttachment(0)->getProperties().format}},
            .depthAttachmentFormat = framebuffer1_->getDepthAttachment()->getProperties().format,
        },
        .cullMode = igl::CullMode::Back,
        .frontFaceWinding = igl::WindingMode::Clockwise,
        .fragmentUnitSamplerMap = {{textureUnit, IGL_NAMEHANDLE("inputImage")}},
  ...
  render(buffer, vb0_, tex0_, pipelineState0_, framebuffer0_, renderPass0_,
         samplerState_, ib0_, textureUnit, backendType,
         fragmentParamBuffer_, fragmentUniformDescriptors_, fragmentParameters_);

  render(buffer, vb1_, tex1_, pipelineState1_, framebuffer1_, renderPass1_,
         samplerState_, ib0_, textureUnit, backendType,
         fragmentParamBuffer_, fragmentUniformDescriptors_, fragmentParameters_);

...
TEST_F(WebPTextureLoaderTest, EmptyBufferFails) {
  std::vector<uint8_t> buffer(kHeaderLength, 0);

  igl::Result ret;
  const auto reader =
      *DataReader::tryCreate(buffer.data(), static_cast<uint32_t>(buffer.size()), nullptr);
...
TEST_F(WebPTextureLoaderTest, PngDataFails) {
  igl::Result ret;
  const auto reader =
      *DataReader::tryCreate(kPngHeader.data(), static_cast<uint32_t>(kPngHeader.size()), nullptr);
  const auto loader = factory_.tryCreate(reader, &ret);
  EXPECT_EQ(loader, nullptr);
  EXPECT_FALSE(ret.isOk());
}
...
TEST_F(WebPTextureLoaderTest, CorruptWebPPayloadFails) {
  ...
  igl::Result ret;
  const auto reader =
      *DataReader::tryCreate(corrupt.data(), static_cast<uint32_t>(corrupt.size()), nullptr);
  const auto loader = factory_.tryCreate(reader, &ret);
  EXPECT_EQ(loader, nullptr);
  EXPECT_FALSE(ret.isOk());
}
...
TEST_F(WebPTextureLoaderTest, MinHeaderLength) {
  EXPECT_EQ(factory_.minHeaderLength(), kHeaderLength);

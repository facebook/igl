...
VulkanStagingDevice::MemoryRegion VulkanStagingDevice::nextFreeBlock(VkDeviceSize size,
  ...
  // Splits `allocatedSize` bytes off the front of `*regionItr`, updates the bookkeeping, and returns
  // the allocated block. Any remainder is written back into `regions_` so it can be reused, and a
  // fully-consumed region is erased. This is the single point at which a region is handed out, so a
  // returned block is never also left in `regions_` as a free region.
  ...
  // After waitAndReset()/allocateStagingBuffer() the deque holds a single whole, ready region. Split
  // it through the same path so the returned block is not also left behind in `regions_` as free.
  // std::min() also covers a contiguous request larger than the maximum staging buffer size (only
  // reachable via alignment padding), handing out the whole buffer instead of nothing.

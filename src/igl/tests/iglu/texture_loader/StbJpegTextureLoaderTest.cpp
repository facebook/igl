...
constexpr const std::array<uint8_t, 67> kSingleBlackPixelGrayscalePNG{
...
constexpr const std::array<uint8_t, 160> kSingleWhitePixelGrayscaleJPG{
    {0xFF, 0xD8, 0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x01, 0x00,
...
TEST_F(StbJpegTextureLoaderTest, ValidDataSucceeds) {
  Result ret;
  auto reader = *iglu::textureloader::DataReader::tryCreate(
...
TEST_F(StbJpegTextureLoaderTest, JpegDataSucceeds) {
  Result ret;
  auto reader = *iglu::textureloader::DataReader::tryCreate(
      kSingleWhitePixelGrayscaleJPG.data(),
      static_cast<uint32_t>(kSingleWhitePixelGrayscaleJPG.size()),
      nullptr);
  auto loader = factory_.tryCreate(reader, &ret);
  EXPECT_NE(loader, nullptr);
  EXPECT_TRUE(ret.isOk()) << ret.message;
...
TEST_F(StbJpegTextureLoaderTest, PngDataFails) {
  Result ret;
  auto reader = *iglu::textureloader::DataReader::tryCreate(
      kSingleBlackPixelGrayscalePNG.data(),
      static_cast<uint32_t>(kSingleBlackPixelGrayscalePNG.size()),
      nullptr);
  auto loader = factory_.tryCreate(reader, &ret);
  EXPECT_EQ(loader, nullptr);
  EXPECT_FALSE(ret.isOk());

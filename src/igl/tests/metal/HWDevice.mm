...
TEST_F(HWDeviceTest, QueryDevicesSanityTest) {
  ...
  HWDeviceQueryDesc queryDesc2(HWDeviceType::Unknown);
  Result result2;

  std::vector<HWDeviceDesc> devices2 = iglHWDev_->queryDevices(queryDesc2, &result2);

  // Currently HWDevice always returns ok when being queried
  ASSERT_TRUE(result2.isOk());

  HWDeviceQueryDesc queryDesc3(HWDeviceType::DiscreteGpu, 8);
  Result result3;

  std::vector<HWDeviceDesc> devices3 = iglHWDev_->queryDevices(queryDesc3, &result3);
...
TEST_F(HWDeviceTest, DeviceCreationFailureTest) {
  ...
  std::unique_ptr<IDevice> device = iglHWDev_->create(deviceDesc, &result);
...
TEST_F(HWDeviceTest, SystemDefaultDeviceCreation) {
  Result result;
  std::unique_ptr<IDevice> device = iglHWDev_->createWithSystemDefaultDevice(&result);
  ASSERT_TRUE(result.isOk());

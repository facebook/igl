...
void PlatformDevice::blitFramebuffer(const std::shared_ptr<IFramebuffer>& src,
  ...
  if (mask & GL_DEPTH_BUFFER_BIT) {
    ...
    if (srcDepthTexture && dstDepthTexture) {
      auto srcFormat = static_cast<igl::opengl::Texture*>(srcDepthTexture.get())
                           ->getGLInternalTextureFormat();
      auto dstFormat = static_cast<igl::opengl::Texture*>(dstDepthTexture.get())
                           ->getGLInternalTextureFormat();
  ...
  if (mask & GL_STENCIL_BUFFER_BIT) {
    ...
    if (srcStencilTexture && dstStencilTexture) {
      auto srcFormat = static_cast<igl::opengl::Texture*>(srcStencilTexture.get())
                           ->getGLInternalTextureFormat();
      auto dstFormat = static_cast<igl::opengl::Texture*>(dstStencilTexture.get())
                           ->getGLInternalTextureFormat();
      if (srcFormat != dstFormat) {
        IGL_DEBUG_ABORT(

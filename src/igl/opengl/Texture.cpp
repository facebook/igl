...
TextureFormat Texture::glInternalFormatToTextureFormat(GLuint glTexInternalFormat,
  ...
  case GL_RED:
    if (glTexFormat == GL_RED) {
      if (glTexType == GL_UNSIGNED_BYTE) {
  ...
  case GL_BGR:
    if (glTexFormat == GL_BGR) {
      if (glTexType == GL_UNSIGNED_SHORT_5_6_5) {
  ...
  case GL_RGBA:
    if (glTexFormat == GL_RGBA) {
      if (glTexType == GL_UNSIGNED_BYTE) {
        return TextureFormat::RGBA_UNorm8;
      } else if (glTexType == GL_UNSIGNED_SHORT_5_5_5_1) {
        return TextureFormat::R5G5B5A1_UNorm;
      } else if (glTexType == GL_HALF_FLOAT_OES) {
        return TextureFormat::RGBA_F16;
      } else if (glTexType == GL_UNSIGNED_INT_8_8_8_8_REV) {
        return TextureFormat::BGRA_UNorm8_Rev;
      } else if (glTexType == GL_UNSIGNED_INT_2_10_10_10_REV) {
  ...
  case GL_RGB_RAW_422_APPLE:
    if (glTexType == GL_UNSIGNED_SHORT_8_8_APPLE) {
      return TextureFormat::R4G2B2_UNorm_Apple;
    } else if (glTexType == GL_UNSIGNED_SHORT_8_8_REV_APPLE) {
  ...
  case GL_DEPTH_COMPONENT:
    if (glTexFormat == GL_DEPTH_COMPONENT) {
      if (glTexType == GL_UNSIGNED_SHORT) {
        return TextureFormat::Z_UNorm16;
      } else if (glTexType == GL_UNSIGNED_INT) {
  ...
  case GL_DEPTH_STENCIL:
    if (glTexFormat == GL_DEPTH_STENCIL) {
      if (glTexType == GL_FLOAT_32_UNSIGNED_INT_24_8_REV) {
        return TextureFormat::S8_UInt_Z32_UNorm;
      } else if (glTexType == GL_UNSIGNED_INT_24_8) {
  ...
  case GL_STENCIL_INDEX:
    if (glTexFormat == GL_STENCIL_INDEX) {
      if (glTexType == GL_UNSIGNED_BYTE) {
        return TextureFormat::S_UInt8;
      }
    }
